...
void PlayerbotHolder::OnBotLogin(Player * const bot)
{
	...
	if (sPlayerbotAIConfig.IsInRandomAccountList(account))
	{
		sLog->outBasic("%d/%d Bot %s logged in", playerBots.size(), sRandomPlayerbotMgr.GetMaxAllowedBotCount(), bot->GetName().c_str());
	}
	else sLog->outBasic("Bot %s logged in", bot->GetName().c_str());
}
...
list<string> PlayerbotHolder::HandlePlayerbotCommand(char const* args, Player* master)
{
	...
	for (set<string>::iterator i = bots.begin(); i != bots.end(); ++i)
	{
		...
		uint64 member = sWorld->GetGlobalPlayerGUID(bot);
		if (!member)
		{
